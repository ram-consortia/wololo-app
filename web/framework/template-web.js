...
template.defaults.imports.dateFormat2 = function(s_date, s_type) {
    ...
    if (o_date.getFullYear() == o_dateNew.getFullYear() && o_date.getMonth() == o_dateNew.getMonth() && o_date.getDate() == o_dateNew.getDate()) { /*今天内*/
        if (s_difference < 60 * 1000) { /*再一分钟之内*/
            return parseInt(s_difference / 1000)+1 + '秒前';
        } else if (s_difference < 60 * 60 * 1000) { /*一个小时内*/
            return parseInt(s_difference / 1000 / 60) + '分钟前';
        } else if (s_difference < 24 * 60 * 60 * 1000) { /*一天内*/
            return parseInt(s_difference / 60 / 60 / 1000) + '小时前';
        }
    } else if (o_date.getFullYear() == o_dateNew.getFullYear() && o_date.getMonth() == o_dateNew.getMonth() && (o_dateNew.getDate() - o_date.getDate()) == 1) { /*昨天内*/
        return '昨天 ' + o_date.getHours() + ":" + o_date.getMinutes();
    } else if (o_date.getFullYear() == o_dateNew.getFullYear() && o_date.getMonth() == o_dateNew.getMonth() && (o_dateNew.getDate() - o_date.getDate()) == 2) { /*前天内*/
        return '前天 ' + o_date.getHours() + ":" + o_date.getMinutes();
    } else {
