...
// 多语言切换方法
function fnLanguage_public(_obj, _cb) {
    if (fnHasEmpty_public(_obj)) {
        _obj = {};
    } else if (typeof _obj === 'function') {
        _cb = _obj;
        ...
        if( fnHasLanguageCN_public() ){
            ...
            if( _text in _json ){
                ...
                if(  _languageName in _json[_text]  ){
                    return _json[_text][_languageName];
                }else{
                    return _text;
                }
            }else{
...
function fnGetLanguageName_public(){
    ...
    var _data = fnCookie(APP.LC_NAME.LANGUAGE),
        _languageName = (navigator.language || navigator.browserLanguage).toLowerCase() || 'zh-cn';
    if (_data) {
        _languageName = _data.value;
    }
...
function fnBackLogin_public(){
    ...
    api.closeToWin({
        name: fnGetPathName_public(APP.PAGE_PATH.SIGN_IN),
        animation: {
            subType: 'from_bottom'
        }
    });
...
function fnVerifyInput_public(type) {
    if (type === 'Phone') {                                     //手机号
        ...
        return /\d*/;
    } else if (type === 'Alphanumeric') {                       //数字或者字母
        return /[0-9a-zA-Z]*/;
...
// 组装一个 get 请求参数
function fnConvertParam_public(param) {
    var _param = [],
        symbol = '?';
...
function fnAjax(_obj, _cb) {
    ...
    fnCookie(APP.LC_NAME.USER, function(_user) {
        ...
        _obj.data.user_type = APP.USER_TYPE.user.value;
        ...
        function _fnReq(_sign) {
            api.ajax(_param, function(ret, err) {
                ...
                // debugAlert('Ajax 原始信息: ' +_obj.url +' >> ', ret, err);
                // 苹果对json数据可能会乱序，所以将返回的数据专为 json
                if (ret) {
                ...
                if (ret) {
                    ...
                    // if(ret.status == APP.HTTP_STATUS.ERROR){
                    //     if( ret.error && ret.error.code == APP.ERROR_CODE.TOKEN_INVALID){
                    //         fnAlert_public({
                    //             title: '提示',
                    ...
                    //
                    //                     // fnRmCookie('HAS_AJAX');
                    //                     fnRmCookie('USER', function(){
                    //
                    //                         fnRmCookie('POP');
                    //                         // 登录失效则跳转到登录页
                    //                         if( api.winName != 'signIn_win' ){
                    //                             fnClose({
                    //                                 name: 'signIn_win'
                    //                             });
                    //                         }
                    ...
                    if(ret.code == APP.ERROR_CODE.TOKEN_INVALID){
                        ...
                        fnAlert_public({
                            title: '提示', //可选 |    ''     | string     |  弹窗标题文案，不传则不显示弹窗顶部文案
                            msg: '该用户已在其它设备登录1', //可选 |    ''     | string     |  弹窗内容 （可解析html格式内容），不传则不显示内容
                            buttons: [{ //必选 |    []      | Array     |  弹窗底部按钮，至少传入一个按钮（数组有多少就显示多少按钮）
                    ...
                    // } else {
                    // if( [500].indexOf(err.statusCode) !== -1 ){
                    //     fnToast(err.body);
                    // }
                    // _fnJumpConnection();
                    // if( [401, 490, 491].indexOf(err.statusCode) !== -1 ){
                    //     fnCookie('HAS_AJAX', true);
                    //     fnAlert_public({
                    //         title: '提示',
                    ...
                    // }else{
                    //     // _callback(ret, err);
                    //     fnToast(err.body.error.message, function(){
                    //
                    //     });
                    // }
                    // }
...
function fnShowProgress_public(_txt, _path) {
    // if (!window['vSetCustomRefreshHeaderInfo']) {
    //     window['vSetCustomRefreshHeaderInfo'] = undefined;
    // }
    var _param = {
        ...
        function _fnRequest(){
            ...
            if( _info[_obj.names[_num]] ){
                if( fnHasEmpty_public(_obj.isTip) || _obj.isTip ){
                    _alert(['没有',  _info[_obj.names[_num]], '权限，是否前去开启？'].join(''));
                }else{
...
function fnCookie(_key, _value, cb) {
    ...
    if (typeof _value != 'undefined' && typeof _value != 'function') {
        debugAlert('设置缓存前', _key, localStorage.getItem(_key))
        ...
        if ([null, 'remove'].indexOf(localStorage.getItem(_key)) != -1) {
            ...
            var _result = JSON.parse(localStorage.getItem(_key));
        }
...
function fnFocus_public(_selector) {
    ...
    setTimeout(function(){
        _el.focus();
        api.require('softInputMgr').toggleKeyboard();
    }, 500);
}
...
var FACEBOOK = {
    ...
    isInstalled: function(_obj, _cb) {
        ...
        FACEBOOK.init().isInstalled(function(ret, err) {
            ...
            if (ret.installed) {
                ...
                if (!_obj.no_tips) {
                    fnToast(fnLanguage_public('当前设备未安装facebook客户端'));
                }
            }
        });
    },
    ...
    getUserInfo: function(_cb) {
        FACEBOOK.init().getUserInfo(function(ret) {
            var _data = {};
            if (ret.status) {
                _data = ret.result;
            }
...
function fnRefresh(_cb) {
    ...
    //     image: {
    //         pull: [
    //             'widget://image/refresh_pull/1.png',
    //             'widget://image/refresh_pull/2.png',
    //             'widget://image/refresh_pull/3.png',
    //             'widget://image/refresh_pull/4.png',
    //             'widget://image/refresh_pull/5.png',
    //             'widget://image/refresh_pull/6.png',
    //             'widget://image/refresh_pull/7.png',
    //             'widget://image/refresh_pull/8.png',
    //             'widget://image/refresh_pull/9.png',
    //             'widget://image/refresh_pull/10.png',
    //             'widget://image/refresh_pull/11.png',
    //             'widget://image/refresh_pull/12.png',
    //             'widget://image/refresh_pull/13.png',
    //             'widget://image/refresh_pull/14.png',
    //             'widget://image/refresh_pull/15.png',
    //             'widget://image/refresh_pull/16.png',
    //             'widget://image/refresh_pull/17.png'
    //         ],
    //         load: [
    //             'widget://image/refresh_pull/1.png',
    //             'widget://image/refresh_pull/2.png',
...
var AJPUSH = {
    ...
    setBadge: function(_obj, _cb) {
        ...
        debugAlert('极光设置角标', _param)
        AJPUSH.init().setBadge(_param);
    },
    bindAliasAndTags: function(_obj, _cb) {
        ...
        debugAlert('极光bindAliasAndTags开始', _obj)
        AJPUSH.init().bindAliasAndTags(_obj, function(ret){
            debugAlert('极光bindAliasAndTags结束', arguments)
            _cb && _cb(ret);
        });
    }
}
...
function fnOpenLinkageWindow_public(_obj, _cb) {
    ...
    if( _obj.headBarSelector ){
        api.addEventListener({
            ...
            debugAlert('fnOpenLinkageWindow_public底部导航切换事件结束', arguments)
            var _data = ret.value,
...
function fnGetLocation_public(_obj, _cb) {
    ...
    function _start() {
        ...
        _fn();
        // api.startLocation(_param, function(ret, err) {
        //     api.stopLocation();
        //     clearTimeout(_timer);
        //     debugAlert('手机自带获取经纬度结束', arguments)
        //     if (ret.status) {
        //         if (ret.latitude && ret.longitude) {
        //             _callback({
        //                 lat: ret.latitude,
